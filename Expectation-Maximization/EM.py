...
def init(points_list, k):
    ...
    mu = np.random.uniform(low=min(points_list), high=max(points_list), size=(k,))
    sigma = np.random.uniform(low=min(points_list), high=max(points_list), size=(k,))

    return w, mu, sigma

def normal_pdf(x, mu, std):
    deno = np.power(2*np.pi*std**2,0.5)
    e_power = -1*np.power(x-mu,2)/(2*std**2)
...
def expectation(points_list, mu, sigma, w):
    ...
    for point in points_list:
        r_point = [w[j]*normal_pdf(point,mu[j], sigma[j]) for j in range(len(w))]
        responsibilities.append(r_point)
    likelihood = np.array(responsibilities)
...
def maximization(points_list, ranks):
    ...
    sum = []
    for col in ranks.T:
        sum.append(np.dot(col, points_list))
    mu_new = np.array(sum)/(len(points_list)*w_new)
    
    ## compute sigma_new ##
    sum = []
    for i,col in enumerate(ranks.T):
        temp = (points_list - mu_new[i])**2
        sum.append(np.dot(temp,col))
    sigma_new = np.array(sum)/(len(points_list)*w_new)
    sigma_new = np.power(sigma_new, 0.5)
...
def calc_max_delta(old_param, new_param):
    ...
    diffs = np.abs(old_param-new_param)
...
def expectation_maximization(points_list, k, max_iter, epsilon):
    ...
    while delta > epsilon and iter_num <= max_iter:
        ...
        # M step
        ranks = likelihood/likelihood.sum(axis=1)[:,None]
